...
- **Physics** (`ballHitsBlock`, `moveBall`, `enforceAngle`) — circle/rectangle collision on the axis of least penetration; reflects the ball and, when asked, chips the block (during a hunt it just reports which block was touched so the game can dive in). After every bounce `enforceAngle` keeps the heading ≥ `MIN_AXIS_ANGLE` off the axes so balls never get stuck near-horizontal/vertical. Only **one** ball roams the meta during a hunt.
